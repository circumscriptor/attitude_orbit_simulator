...
auto permanent_magnet::compute_potential_energy(const vec3& b_field_body) const -> double {
    return -_magnetic_moment_body.dot(b_field_body);
}

auto permanent_magnet::compute_magnetic_moment_at_temperature(double temp_celsius, double temp_coeff, double temp_ref) -> vec3 {
    const double temp_factor        = 1.0 + (temp_coeff * (temp_celsius - temp_ref));
...
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
auto permanent_magnet::compute_magnetic_moment(double      remanence,
                                               double      volume,
    ...
    const double chi_m             = relative_permeability - 1.0;
    const double magnetization_eff = magnetization_raw / (1.0 + demagnitization_factor * chi_m);
    const double moment_mag        = magnetization_eff * volume;
