...
inertia_tensor::inertia_tensor(real mass_kg, const spacecraft_shape& shape) {
    std::visit(
        [&](auto&& shape) {
...
inertia_tensor::inertia_tensor(real mass_kg, const spacecraft_uniform& shape)
    : _inertia_tensor_kg_m2(compute_inertia_tensor(mass_kg, shape.dimensions_m.x(), shape.dimensions_m.y(), shape.dimensions_m.z())),
      _inertia_tensor_kg_m2_inverse(_inertia_tensor_kg_m2.inverse()) {}
...
auto inertia_tensor::compute_gravity_gradient_torque(const vec3& r_body, real earth_mu) const -> vec3 {
    const real r_sq  = r_body.squaredNorm();
    const real r_mag = std::sqrt(r_sq);

    // tau_gg = (3 * mu / r^5) * (r_body × (I * r_body))
    const real coef = (3.0 * earth_mu) / (r_sq * r_sq * r_mag);
    return coef * r_body.cross(_inertia_tensor_kg_m2 * r_body);
}

auto inertia_tensor::compute_inertia_tensor(real m_kg, real a, real b, real c) -> mat3x3 {
    const real i_x = (1. / 12.) * m_kg * (b * b + c * c);
    const real i_y = (1. / 12.) * m_kg * (a * a + c * c);
    const real i_z = (1. / 12.) * m_kg * (a * a + b * b);
