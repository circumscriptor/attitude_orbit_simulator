...
void aos::simulation::spacecraft_dynamics::operator()(const system_state& current_state, system_state& state_derivative, double t) const {
    ...
    const vec3 b_inertial = m_environment->inertial_magnetic_field_at(t);
    ...
    total_torque_body += m_spacecraft->magnet().magnetic_moment().cross(b_body);

    // b) Torque from all hysteresis rods
    for (std::size_t i = 0; i < m_spacecraft->rods().size(); ++i) {
        total_torque_body += m_spacecraft->rods()[i].magnetic_moment(current_state.rod_magnetizations(static_cast<int>(i))).cross(b_body);
    }
