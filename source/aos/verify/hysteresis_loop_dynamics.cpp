...
hysteresis_loop_dynamics::hysteresis_loop_dynamics(const hysteresis_rod& rod, real h_max, real frequency) : _rod(&rod), _h_max(h_max), _frequency(frequency) {}

auto hysteresis_loop_dynamics::h_max() const noexcept -> real {
    return _h_max;
}

auto hysteresis_loop_dynamics::frequency() const noexcept -> real {
    return _frequency;
}

void hysteresis_loop_dynamics::operator()(const hysteresis_state_type& m, hysteresis_state_type& dm_dt, real t) const {
    // Applied H-field and its time derivative (Sinusoidal Drive)
    const real h     = _h_max * std::sin(2.0 * pi * _frequency * t);
    const real dh_dt = _h_max * 2.0 * pi * _frequency * std::cos(2.0 * pi * _frequency * t);
