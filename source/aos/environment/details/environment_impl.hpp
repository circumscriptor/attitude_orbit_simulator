...
public:
    ...
    [[nodiscard]] auto compute_effects(real_t t_sec, const vec3& r_eci_m, const vec3& v_eci_m_s) const -> environment_effects override;
...
protected:
    ...
    [[nodiscard]] auto atmospheric_density() const -> real_t;
    ...
    void cache_transform(real_t t_sec, const vec3& r_eci_m) const;

    [[nodiscard]] static auto earth_relative_v(const vec3& v_eci_m_s, const vec3& r_eci_m) -> vec3;

    [[nodiscard]] static auto sun_position_eci(real_t days_since_j2000) -> vec3;

    [[nodiscard]] static auto solar_perturbation(const vec3& r_sat_eci, const vec3& r_sun_eci) -> vec3;

    [[nodiscard]] static auto earth_shadow_factor(const vec3& r_sat, const vec3& r_sun) -> real_t;

private:

    real_t                       _start_year_decimal;
