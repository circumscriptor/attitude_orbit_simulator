...
auto environment_impl::compute_effects(real_t t_sec, const vec3& r_eci_m, const vec3& v_eci_m_s) const -> environment_effects {
    ...
    const real_t d_sun_sq = r_sun.squaredNorm();
    const real_t pressure = solar_pressure_1au * (au_to_m_2 / d_sun_sq);
    const real_t shadow   = earth_shadow_factor(r_eci_m, r_sun);
    const real_t earth_mu = _gravity_model.MassConstant();

    // compute fields at future position for gradient calculation
    const real_t t_next = t_sec + dt_gradient;
...
void environment_impl::cache_transform(real_t t_sec, const vec3& r_eci_m) const {
    _cache.current_year = _start_year_decimal + (t_sec / seconds_per_year);

    const real_t days_j2000 = (_cache.current_year - 2000.0) * 365.25;
    _cache.r_sun_eci        = sun_position_eci(days_j2000);

    const real_t theta     = earth_rotation_rate_rad_s * t_sec;  // TODO: calculate gmst
...
auto environment_impl::atmospheric_density() const -> real_t {
...
auto environment_impl::sun_position_eci(real_t days_since_j2000) -> vec3 {
    const real_t g       = (357.528 + 0.9856003 * days_since_j2000) * deg_to_rad;
    const real_t l       = (280.460 + 0.9856474 * days_since_j2000) * deg_to_rad;
    const real_t sin_g   = std::sin(g);
    const real_t cos_g   = std::cos(g);
    const real_t lambda  = l + ((1.915 * sin_g + 0.020 * (2.0 * sin_g * cos_g)) * deg_to_rad);
    const real_t epsilon = (23.439 - 0.0000004 * days_since_j2000) * deg_to_rad;
    const real_t r_au    = 1.00014 - (0.01671 * cos_g) - (0.00014 * (2.0 * cos_g * cos_g - 1.0));
    const real_t r_m     = r_au * au_to_m;
    const real_t sin_l   = std::sin(lambda);
    const real_t cos_l   = std::cos(lambda);
    const real_t sin_e   = std::sin(epsilon);
    const real_t cos_e   = std::cos(epsilon);
    return {
        r_m * cos_l,
        r_m * cos_e * sin_l,
        r_m * sin_e * sin_l,
...
auto environment_impl::solar_perturbation(const vec3& r_sat_eci, const vec3& r_sun_eci) -> vec3 {
    ...
    const real_t d_rel_sq = r_rel.squaredNorm();
    const real_t d_sun_sq = r_sun_eci.squaredNorm();
    // (d^2)^(1.5) = d^3
    const real_t d_rel_cubed = d_rel_sq * std::sqrt(d_rel_sq);
    const real_t d_sun_cubed = d_sun_sq * std::sqrt(d_sun_sq);
    // a = mu * (r_rel/d_rel^3 - r_sun/d_sun^3)
    return sun_mu_m3_s2 * ((r_rel / d_rel_cubed) - (r_sun_eci / d_sun_cubed));
}

auto environment_impl::earth_shadow_factor(const vec3& r_sat, const vec3& r_sun) -> real_t {
    const real_t d_sat     = r_sat.norm();
    const vec3   unit_sat  = r_sat / d_sat;
    const vec3   unit_sun  = r_sun.normalized();
    const real_t cos_theta = unit_sat.dot(unit_sun);
    if (cos_theta > 0) {
        return 1.0;  // Sun is on the same side as satellite
    }

    const real_t d_sat_sun = (r_sun - r_sat).norm();

    // Apparent angular radii (half-angles)
    const real_t alpha = std::asin(sun_radius_m / d_sat_sun);
    const real_t beta  = std::asin(earth_radius_m / d_sat);
    const real_t gamma = std::acos(std::clamp(-cos_theta, -1.0, 1.0));
