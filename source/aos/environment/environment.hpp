...
struct environment_effects {
    ...
    real_t atmospheric_density_kg_m3;   // [kg/m^3] Density of gasses including Anomalous Oxygen
    vec3   r_sun_eci;                   // [m] Position of Sun relative to Earth
    vec3   v_earth_rel;                 // [m/s] Earth-Spacecraft relative speed
    real_t shadow_factor;               // [-] 1.0 = Sun, 0.0 = Umbra, (0,1) = Penumbra
    real_t solar_pressure_Pa;           // [N/m^2] Radiation pressure at current distance
    real_t earth_mu;                    // [?] Earth mass
    // NOLINTEND(readability-identifier-naming)
};

struct environment_properties {
    real_t      start_year_decimal;
...
public:
    ...
    [[nodiscard]] virtual auto compute_effects(real_t t_sec, const vec3& r_eci_m, const vec3& v_eci_m_s) const -> environment_effects = 0;
